"""
This python code reads in MPAS data and passes it to the "blossom" program to train a SOM. 
...
User settings
"""
# Grab desired MPAS files using glob.glob*: here, I am getting DJF dates from the water years 2000 - 2009
# *make sure to use the sorted() function!!
mpas_files = sorted(glob.glob("/cw3e/mead/projects/csg101/output/nosoil/WY200*/latlon/latlon*-12-*_00:00:00*")) + sorted(glob.glob("/cw3e/mead/projects/csg101/output/nosoil/WY200*/latlon/latlon*-01-*_00:00:00*")) + sorted(glob.glob("/cw3e/mead/projects/csg101/output/nosoil/WY200*/latlon/latlon*-02-*_00:00:00*"))
print(len(mpas_files))

#Select a pressure level in hPa
p_level = 500

#Variable to train the SOM on
var_name = 'height'
...
dates = [] #List of dates to be used later on
for i in range(len(mpas_files)): #Loop through each output file
    print(str(i) + "/" + str(len(mpas_files)))
    ncfile = mpas_files[i]
    ...
    ds_sub = ds.sel(lev=p_level, time = times[0]) #Subset data by pressure level and time
    lats = ds_sub['lat'].values
    lons = ds_sub['lon'].values
    variable = ds_sub[var_name].values
...
q = np.zeros([6])
t = np.zeros([6])
rows_arr = np.arange(2,5,1)
cols_arr = np.arange(2,5,1)
for i in rows_arr:
...
ax2 = ax1.twinx()
ax1.plot(np.arange(1,7,1), q, color = 'blue', label = "Quantization error")
ax2.plot(np.arange(1,7,1), t, color = 'red', label = "Topographic error")
...
"""
#Grabbing the landmask to use as a coastline
ds = xr.open_dataset('/cw3e/mead/projects/csg101/output/nosoil/WY2004/latlon/latlon.15km.nature.2004-06-01_00:00:00.nc')
times = ds['time'].values
ds = ds.where((ds['lat']<nlat) & (ds['lat']>slat) &  (ds['lon']>wlon) & (ds['lon']<elon), drop = True)
ds = ds.sel(lev=p_level, time = times[0])
xland = ds['xland'].values

som_data = SOM.comp_data #Get the composite data for visualization
...
for x in range(0,SOM.rows):
    for y in range(0,SOM.cols):
        ...
        axs[x,y].contour(lons, lats, xland, transform=ccrs.PlateCarree(), colors = 'black')
        cs = axs[x,y].contourf(lons, lats, data, cmap = 'jet', transform=ccrs.PlateCarree(), levels = 30)#np.arange(4900, 6000, 30))
        mapnum = mapnum + 1
